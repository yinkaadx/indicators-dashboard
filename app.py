...
te_api_key = "e8ebf3567f8b473:l8zd23rsl5xzf55"  # Your Trading Economics API key
...
def fetch_data(indicator):
    ...
    try:
        ...
        url = f"https://api.tradingeconomics.com/markets?countries=united-states&c={te_indicator}&outtype=json&api_key={te_api_key}"
        response = requests.get(url)
        if response.ok:
            data_json = response.json()
            for item in data_json:
                if "Previous" in item:
                    data["previous"] = float(item["Previous"]) if item["Previous"] else data["previous"]
                if "Last" in item:
                    data["current"] = float(item["Last"]) if item["Last"] else data["current"]
                if "Forecast" in item:
                    data["forecast"] = float(item["Forecast"]) if item["Forecast"] else data["forecast"]
        # Scrape fallback for non-API (e.g., Power index)
        else:
            ...
            if url:
                ...
                data["current"] = np.nan  # Parse needed
...
for ind in selected:
    ...
    with col2:
        ...
        threshold_value = float(THRESHOLDS.get(ind, "").split()[0].strip('><%')) if THRESHOLDS.get(ind) and THRESHOLDS.get(ind).split() else np.nan
